...
uses
  ...
  StdCtrls, ExtCtrls, MMSystem, jpeg, ComCtrls, XPman;
...
var
  Form1: TForm1;
  SFrame1,SFrame2,BFrame,
  x1,y1,x2,y2,CurX,CurY,
  Counter,Hits,munition:Integer;
  flipX1,flipY1,flipX2,FlipY2,
  hit1,hit2,Start:Boolean;
...
begin
  ...
  Buffer.Canvas.TextOut(15,12,'Elapsed Time: '+IntToStr(counter));
  Buffer.Canvas.TextOut(120,12,'Number: '+IntToStr(hits));
  Buffer.Canvas.TextOut(200,12,'Ammunition: '+IntToStr(munition));
  Buffer.Canvas.TextOut(320,12,FormatDateTime('hh:mm:ss',Time));
  if ((Counter<=0) or (munition<=0)) and Start then
  begin
    StopGame;
  ...
  if flipy1 then
    ...
    y1:=y1+4;
  if x1>=buffer.width-48 then
    flipx1:=True
  else if x1<=0 then
    flipx1:=False;
  if y1>=buffer.Height-35 then
    flipy1:=True
  else if y1<=30 then flipy1:=False;
  end;
    DrawSprite(buffer.canvas,spr1,spr1,x1,y1,Sframe1);
  end else
    DrawBlood(buffer.canvas,blood,blood,x1,y1,Bframe);
  ...
  begin
    ...
    begin
      ...
      DrawCursor;
      Form1.Canvas.Draw(0,0,buffer.Picture.Graphic);
end;
