...
begin
    ...
    begin
      ...
      for j:=1 to 8 do
        ...
        begin
          case feld[i,j] of
           ' ' : brush.color:=clwhite;
           'x' : begin
                   bitmap.canvas.draw(xoffset+(i-1)*64+1,
                                      yoffset+(j-1)*64+1,
      ...
      'a'..'m' : begin
          ...
          rectangle(xoffset+(i-1)*64,yoffset+(j-1)*64,
                                     xoffset+i*64+1,
                                     yoffset+j*64+1);
          ...
          begin
            ...
            font.color:=clwhite;
            textout(xoffset+(i-1)*64+32-textwidth(ks) div 2,
                                      yoffset+(j-1)*64+8,ks);
          end;
        end;
    end;
...
begin
    ...
    Button3.visible:=length(loesung)>0;
    if length(loesung)>0 then label2.caption:='Step count '+
                                    inttostr(length(loesung) div 3)
                         else label2.caption:='Step count ?';
...
begin
    ...
    begin
      if spunkte<0 then spunkte:=0;// spoints+1;
      if (spunkte>0) then
      begin
        Application.CreateForm(TForm2, Form2);
        Form2.showmodal;
      ...
      lebenszahl:=8;
      spielstufe:=1;
      Edit1.text:=inttostr(spielstufe);
      l1.caption:='Game level '+inttostr(spielstufe);
    end;
    fillchar(feld,sizeof(feld),' ');
    k:='['+inttostr(spielstufe);
...
procedure TForm1.PaintBox1MouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
var
  p,q,b,h,xoffset,yoffset,j:integer;
  leer:boolean;
begin
   if verschieben then
   begin
...
procedure TForm1.PaintBox1MouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  p,q,b,h,kk,xoffset,yoffset,i,j,ii,eins:integer;
  leer:boolean;
  zeichen:char;
begin
   if verschieben then
   begin
     ...
     begin
       ...
       showmessage('Congratulations! Game level '+inttostr(spielstufe)+' is solved!');
...
procedure TForm1.nachrutschen(Sender: TObject);
var
  b,h,xoffset,yoffset,kk,eins,jj,ii,i,i2,j2,j,p,q:integer;
  zeichen:char;
begin
    b:=PaintBox1.width;
    h:=PaintBox1.height;
...
procedure TForm1.automatic(Sender: TObject; p,q: Integer);
var
  b,h,kk,xoffset,yoffset,i,j,k,eins:integer;
  leer:boolean;
  zeichen:char;
begin
    b:=PaintBox1.width;
    ...
    begin
      timer1.enabled:=false;
      application.processmessages;
      Button3.caption:='Demonstration';
      showmessage('The puzzle is solved!');
      d2click(sender);
    end;
end;
...
begin
    ...
    else Button3.caption:='Show solution';
end;
