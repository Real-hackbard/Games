...
uses
  ...
  XPMan, SoundPlayerThread, MMSystem;
...
begin
   ...
   until (k2[1]='[');
   if test<>60 then showmessage('Error: '+lb1.items[sel]+' / '+inttostr(test));
...
begin
  try
    ...
    br:= round(1/2*(PaintBox1.Width/gg-12)+12);
    ho:= round(1/2*PaintBox1.Height/gg);

    bitmap.canvas.pen.color:=$0e0e0e0; // clltgray;
    i:=0;
    repeat
      bitmap.canvas.moveto(i*gg,-1);
      bitmap.canvas.lineto(i*gg,PaintBox1.height+1);
      inc(i);
    until i*gg>PaintBox1.width;
    i:=0;
    repeat
      bitmap.canvas.moveto(-1,i*gg);
      bitmap.canvas.lineto(PaintBox1.width+1,i*gg);
      inc(i);
    until i*gg>PaintBox1.height;
...
var
  ms1: TResourcestream; ms2: TMemoryStream;
begin
...
begin
    ...
    gg:=round(28*PaintBox1.height/573);
    lb1.clear;
    nr:=0;
    repeat
      k:=liste2[nr];
    ...
    until (k[1]='[');
    lb1.itemindex:=0;
    l1.caption:=inttostr(lb1.items.count)+' Tasks';
    k1:=lb1.items[lb1.itemindex];
...
var
  ...
  x1,y1:integer;
begin
    farbx:=PaintBox1.canvas.pixels[x,y];
    teilnummer:=0;
    for i:=1 to anzahl do
      ...
      begin
        ...
        ms.LoadFromFile(ExtractFilePath(Application.ExeName) + 'Sounds\4.wav');
        ms.Position := 0;
        PlayWaveStream(ms);
      end;
...
begin
    ...
    begin
      ...
      ms.LoadFromFile(ExtractFilePath(Application.ExeName) + 'Sounds\5.wav');
      ms.Position := 0;
      PlayWaveStream(ms);
    end;
end;
...
begin
    ...
    begin
      ...
      ms.LoadFromFile(ExtractFilePath(Application.ExeName) + 'Sounds\1.wav');
      ms.Position := 0;
      PlayWaveStream(ms);
      if geschafft then
        showmessage('Congratulations. The solution is correct.');
    end;
end;
