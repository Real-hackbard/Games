...
uses
  ...
  Dialogs, ExtCtrls, Jong, Astar, StdCtrls, Menus, dxGDIPlusClasses,
  XPMan;
...
begin
  // Initializes the path search lists
  ...
  nivo := 0;
  lim := 240;   // Maximum game duration in seconds for a table
end;
...
procedure TForm1.AfficheCase(x,y,d : integer); // for the path's layout
begin
...
begin
  ...
  if cas1 then            // if you click on the first box
  begin
    depx := X div 60;
    depy := Y div 60;
    px := depx * 60 + 1;
  ...
  Plato.Canvas.Polyline([Point(px,py),Point(px+57,py),Point(px+57,py+57),
                          Point(px,py+57),Point(px,py)]);
  if tablo[butx,buty] <> tablo[depx,depy] then   // if different boxes
  begin
    beep;
  ...
  d := tablo[depx,depy];    // we save the case type
  tablo[depx,depy] := -1;   // We cancel the start and end fields to allow
  tablo[butx,buty] := -1;   // finding the path composed of negative squares
  if CheminOk(Point(depx,depy),Point(butx,buty)) then  // If a path exists
  begin
    Plato.Canvas.Draw(depx*60,depy*60,cases[16]);
    for i := High(chemin) downto 1 do                  // we display it
    begin
      AfficheCase(chemin[i].X,chemin[i].Y,d);
    end;
    image.Canvas.Draw(depx*60,depy*60,cases[16]);      // and we erase the boxes
    ...
    begin   // otherwise we restore the start and end path fields
      beep;
      tablo[depx,depy] := d;
      tablo[butx,buty] := d;
      Plato.Canvas.Draw(0,0,image);
...
begin
  k := 0;
  for i := 1 to 32 do     // drawing pairs of squares
  begin
    tca[i] := k;
    tca[i+32] := k;
    inc(k);
  ...
  c := 0;
  for x := 1 to 8 do      // display of boxes
    for y := 1 to 8 do
    begin
      inc(c);
      tablo[x,y] := tca[c];
  ...
  inc(nivo);
  Pnivo.Caption := Format('Level %d',[nivo]);
  Timer1.Enabled := true;
end;
...
begin
  ...
  if tmp < lim then
  begin
    case nivo of          // The duration is reduced according to the level
      1 : dec(lim,30);
      2 : dec(lim,25);
      3 : dec(lim,20);
      4 : dec(lim,15);
    ...
    Initialiser;          // we launch the following table
  end;
end;
