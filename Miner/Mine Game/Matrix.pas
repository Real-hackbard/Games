...
{ A unit for manipulating and calculating two-dimensional arrays.
  Primary use: operation of a Minesweeper-type program.
  Some routines have more general uses. }

interface

type
  TMatrixBool = array of array of Boolean;  // two-dimensional array of booleans
  TMatrixByte = array of array of Byte;     // two-dimensional array of bytes

  { Dimensioning the Boolean array in two dimensions }
  procedure SetLengthMatrixBool(var MatrixBool: TMatrixBool; Horz, Vert: Integer);

  { Dimensioning the byte array in two dimensions }
  procedure SetLengthMatrixNumb(var MatrixByte: TMatrixByte; Horz, Vert: Integer);

  { Initializing the boolean array (default value = False) }
  procedure InitializeMatrixBool(var MatrixBool: TMatrixBool;
    const Value: Boolean = False);

  { Initializing the byte array (default value = 0) }
  procedure InitializeMatrixByte(var MatrixByte: TMatrixByte;
    const Value: Byte = 0);

  { Returns the number of cells from a boolean array in a random order. }
  procedure GetRandomCells(var MatrixBool: TMatrixBool; Num: Word);

  { Counting the mines adjacent to each cell of the table }
  procedure SearchForNeighboring(var MatrixByte: TMatrixByte; MatrixBool: TMatrixBool);

  { Clearing the unmined space - this procedure is recursive }
  procedure SearchForShowing(var MShow: TMatrixBool; MatrixByte: TMatrixByte;
    x, y: Integer);

  { Returns True if all cells have a value of = True in a boolean array }
  function MatrixBoolIsTrue(var MatrixBool: TMatrixBool): Boolean;

  { Count the number of cells whose value = False in a byte array }
  function CountMatrixBoolFalse(var MatrixBool: TMatrixBool): Word;

  { Count the number of occurrences of Value in a byte array }
  ...
  { Compares values ??in a byte array and in a boolean array }
...
begin
  ...
  { rightmost column without corner cells }
  x := High(MatrixByte);
  for y := 1 to High(MatrixByte[0]) - 1 do
  begin
    Count := 0;
  ...
  { bottom row without corner cells }
  y := High(MatrixByte[0]);
  for x := 1 to High(MatrixByte) - 1 do
  begin
    Count := 0;
  ...
  { left column without corner cells }
  x := 0;
  for y := 1 to High(MatrixByte[0]) - 1 do
  begin
    Count := 0;
...
{ Compares values ??in a byte array and a boolean array.
  Calculates the number of times a match is found between these two values
  at the same coordinates and in their respective arrays, resets the
  byte value ValueA := InitA, and returns the number of occurrences. }
function CompareMatrixByteBool(var MatrixByte: TMatrixByte; ValueA: Byte;
  InitA: Byte; MatrixBool: TMatrixBool; const ValueB: Boolean = False): Word;
var
  x, y: Integer;
begin
  ...
  for x := 0 to High(MatrixByte) do
    for y := 0 to High(MatrixByte[0]) do
      ...
      begin
        MatrixByte[x, y] := InitA;  // reinitialisation
        Inc(Result);
      end;
end;
