...
uses
  ...
  Dialogs, ExtCtrls, Buttons, StdCtrls, XPMan;
...
procedure TForm1.Im1MU(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  col,row:integer;
   p1,p2: integer;
  ...
  if (button=mbleft) and (board[col,row].occupiedby=empty) then begin
    ...
    if board[col,row].occupiedby<>empty then begin
      if sameboard(board,prevboard[playercolor]) and (p2-p1=1) then begin
        ...
        drawAllstones;
        showmessage('Co-train is not possible') ;
      end else begin
...
procedure TForm1.D2C(Sender: TObject);
begin
...
begin
  ...
  playerColor:=white;
  changeplayer;
  hincr:=Image1.width div vlines;
  tokensizex:= hincr div 2 -1;
  vincr:=Image1.height div hlines;
...
begin
  ...
  if playerColor=white then begin
    Label7.caption:='White bets';
    Shape1.brush.color:=clwhite;
  end else begin
    Label7.caption:='Black bets';
...
begin
  ...
  while i<=nbrblocks-1 do
    ...
    if (newscore =0) and (possibleSuicide) then begin
      ...
      showmessage('Suicide move not possible!');
...
procedure TForm1.G1C(Sender: TObject);
begin
  resetboard;
end;

procedure TForm1.D3C(Sender: TObject);
begin
  copyboard(saveboard,board);
...
begin
  If SaveDialog1.execute then begin
    ...
    rewrite(f);
    writeln(f,RadioGroup1.itemindex, ' ', ord(playercolor), ' ',0,
...
procedure TForm1.CB1C(Sender: TObject);
var
...
begin
  ...
  Image1.top:=10;
  Image1.height:=panel2.height-20;
