...
uses
  ...
  Menus, ExtCtrls, Buttons, ToolWin, ComCtrls, StdCtrls;
...
var teile,ziele : array[1..30] of
    ...
    farbensatz : array[1..3,1..3] of integer =
      ...
      ($00000a0,$0000f0,$0090f0),($000a000,$00f000,$90f000));
...
begin
    ...
    for i:=1 to teilezahl do
      reihe[teile[i].rf]:=teile[i].nr;

    for i:=teilezahl downto 1 do
    begin
      ii:=reihe[i];
      x:=(teile[ii].x+xoffset)*wa;
      if odd(teile[ii].x) then
        y:=(teile[ii].y+yoffset)*se+se/2
      else
        y:=(teile[ii].y+yoffset)*se;
