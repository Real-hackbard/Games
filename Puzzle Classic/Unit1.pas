...
uses
  ...
  ExtCtrls, Buttons, ComCtrls, jpeg, StdCtrls, Menus, ExtDlgs, XPMan;
...
begin
    ...
    bitmap.free;
    Label1.caption:='Moves '+inttostr(puzzlezug); korrekt:=0;
    for i:=1 to x12 do
      for j:=1 to y12 do
      begin
        if (puzzlef[i,j].m=i+(j-1)*x12) and (puzzlef[i,j].n=0) then inc(korrekt);
      end;
      Label3.caption:='Correct parts '+inttostr(korrekt);
    if (korrekt=x12*y12) and (not nochkeinbild) then
    begin
      timer1.enabled:=false;
      showmessage('Congratulations! You have correctly assembled the image.!');
      nochkeinbild:=true;
    end;
end;
...
begin
    ...
    begin
      ...
      j1:=1;
      modu:=1;
      if RadioGroup2.itemindex=1 then modu:=4;
      z1:=(x div b12)+1;
      z2:=y div b12;
      z1:=z2*x12+z1;
      for i:=1 to x12 do
...
begin
    ...
    begin
      if OpenPictureDialog1.filterindex=1 then
        ladejpg(OpenPictureDialog1.filename,image3.picture.bitMap);
