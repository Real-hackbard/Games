object Form1: TForm1
  Left = 296
  Top = 215
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Chain Reaction'
  ClientHeight = 673
  ClientWidth = 945
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clBlack
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Menu = MainMenu1
  OldCreateOrder = False
  Position = poScreenCenter
  Scaled = False
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object P5: TPanel
    Left = 0
    Top = 0
    Width = 945
    Height = 673
    Align = alClient
    BevelOuter = bvNone
    Color = clWhite
    Font.Charset = ANSI_CHARSET
    Font.Color = clBlack
    Font.Height = -12
    Font.Name = 'Verdana'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object Panel1: TPanel
      Left = 0
      Top = 0
      Width = 193
      Height = 673
      Align = alLeft
      BevelOuter = bvNone
      Color = 15790320
      TabOrder = 0
      object L1: TLabel
        Left = 8
        Top = 76
        Width = 84
        Height = 14
        Caption = 'Playing field :'
      end
      object L0: TLabel
        Left = 40
        Top = 104
        Width = 48
        Height = 14
        Caption = 'Player :'
      end
      object D3: TButton
        Left = 32
        Top = 28
        Width = 121
        Height = 25
        Caption = 'New Game'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        TabOrder = 6
        OnClick = Button3click
      end
      object Memo2: TMemo
        Left = 8
        Top = 192
        Width = 177
        Height = 177
        BorderStyle = bsNone
        Color = 15790320
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'Verdana'
        Font.Style = []
        Lines.Strings = (
          'Players take turns placing a '
          'ball on their own or an '
          'empty space.'
          ''
          'If there are 4 balls in a '
          'space (3 in edge spaces, 2 '
          'in corner spaces), these '
          'are distributed to the '
          'adjacent '
          'spaces, and that space is '
          'occupied by the current '
          'player.'
          ''
          'The goal is to occupy all '
          'spaces or clear the '
          'opponent'#39's spaces!')
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
      end
      object Panel15: TPanel
        Left = 8
        Top = 384
        Width = 177
        Height = 185
        BevelOuter = bvNone
        Color = 15790320
        TabOrder = 1
        object L2: TLabel
          Left = 40
          Top = 31
          Width = 4
          Height = 14
        end
        object Image2: TImage
          Left = 8
          Top = 24
          Width = 21
          Height = 155
          AutoSize = True
          Picture.Data = {
            
            21C2D29D4B8F3E2D9A346A37034D7A669000003B}
          Transparent = True
        end
        object L4: TLabel
          Left = 40
          Top = 58
          Width = 4
          Height = 14
        end
        object L5: TLabel
          Left = 40
          Top = 85
          Width = 4
          Height = 14
        end
        object L6: TLabel
          Left = 40
          Top = 112
          Width = 4
          Height = 14
        end
        object L7: TLabel
          Left = 40
          Top = 139
          Width = 4
          Height = 14
        end
        object L8: TLabel
          Left = 40
          Top = 166
          Width = 4
          Height = 14
        end
        object Panel16: TPanel
          Left = 0
          Top = 0
          Width = 177
          Height = 17
          Align = alTop
          Alignment = taLeftJustify
          BevelOuter = bvNone
          Caption = 'Score :'
          Font.Charset = ANSI_CHARSET
          Font.Color = clNavy
          Font.Height = -12
          Font.Name = 'Verdana'
          Font.Style = [fsBold]
          ParentColor = True
          ParentFont = False
          TabOrder = 0
        end
        object Pr1: TProgressBar
          Left = 80
          Top = 32
          Width = 86
          Height = 12
          Smooth = True
          Step = 1
          TabOrder = 1
        end
        object Pr2: TProgressBar
          Left = 80
          Top = 59
          Width = 86
          Height = 12
          Smooth = True
          Step = 1
          TabOrder = 2
        end
        object Pr3: TProgressBar
          Left = 80
          Top = 86
          Width = 86
          Height = 12
          Smooth = True
          Step = 1
          TabOrder = 3
        end
        object Pr4: TProgressBar
          Left = 80
          Top = 113
          Width = 86
          Height = 12
          Smooth = True
          Step = 1
          TabOrder = 4
        end
        object Pr5: TProgressBar
          Left = 80
          Top = 140
          Width = 86
          Height = 12
          Smooth = True
          Step = 1
          TabOrder = 5
        end
        object Pr6: TProgressBar
          Left = 80
          Top = 167
          Width = 86
          Height = 12
          Smooth = True
          Step = 1
          TabOrder = 6
        end
      end
      object CB8: TCheckBox
        Left = 16
        Top = 144
        Width = 97
        Height = 17
        Caption = 'Animation'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        TabOrder = 2
      end
      object CB9: TCheckBox
        Left = 16
        Top = 160
        Width = 156
        Height = 17
        Caption = 'Computer opponents'
        Font.Charset = ANSI_CHARSET
        Font.Color = clBlack
        Font.Height = -12
        Font.Name = 'Verdana'
        Font.Style = []
        ParentFont = False
        TabOrder = 3
      end
      object r4: TRxSpinEdit
        Left = 96
        Top = 72
        Width = 49
        Height = 22
        EditorEnabled = False
        MaxValue = 15.000000000000000000
        MinValue = 5.000000000000000000
        Value = 15.000000000000000000
        TabOrder = 4
        OnChange = TrackBar4Change
      end
      object r5: TRxSpinEdit
        Left = 96
        Top = 100
        Width = 49
        Height = 22
        EditorEnabled = False
        MaxValue = 6.000000000000000000
        MinValue = 2.000000000000000000
        Value = 4.000000000000000000
        TabOrder = 5
        OnChange = TrackBar5Change
      end
    end
    object P7: TPanel
      Left = 193
      Top = 0
      Width = 752
      Height = 673
      Align = alClient
      BevelOuter = bvNone
      Color = clWhite
      TabOrder = 1
      object PaintBox1: TPaintBox
        Left = 0
        Top = 0
        Width = 752
        Height = 673
        Align = alClient
        OnMouseDown = PaintBox1MouseDown
        OnPaint = PaintBox1Paint
      end
    end
  end
  object MainMenu1: TMainMenu
    Left = 208
    Top = 32
    object M11: TMenuItem
      Caption = 'Quit'
      ShortCut = 27
      OnClick = S3Click
    end
  end
end
