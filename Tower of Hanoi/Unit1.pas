...
var sabbruch:boolean=true;
    gefunden:boolean=false;
    shoehe:integer=18;

    scheibenzahl:integer;
...
begin
   ...
   begin
     ...
     begin
       ...
       for i:=1 to updown1.position do
        stab[1,i]:=i;
       gesamtzuege:=1;
       for i:=1 to scheibenzahl do
        gesamtzuege:=2*gesamtzuege;
   ...
   begin
     ...
     begin
       if i in [4,7,11,16,22,29,37,46,56] then
        addi:=addi*2;
...
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            nr:=80-bfaktor*wert;
            rectangle(i*b-nr,h-unten-j*shoehe,i*b+nr+1,
                            h-unten-j*shoehe+shoehe-1);
          end;
        end;
      end;
      if gefunden then
      begin
        ...
        pen.color:=clblack;
        pen.width:=2;
        rectangle(move1*b-nr-1,h-unten-j*shoehe-1,move1*b+nr+3,
                              h-unten-j*shoehe+shoehe);
        pen.color:=clblack;
        pen.width:=1;
      end;
    end;
   ...
   begin
     ...
     begin
       ...
       begin
         ...
         dec(stab[move1,0]);
         inc(stab[move2,0]);
         lb2.items.add(inttostr(zuege+1)+#9+chr(64+move1)+' to '+chr(64+move2));
       end
       else
         lb2.items.add('Error: '+chr(64+move1)+' to '+chr(64+move2));
       inc(zuege);
   ...
   begin //3
     ...
     begin
       ...
       then begin
         ...
         lb2.items.add('Fehler: '+chr(64+move1)+' to '+chr(64+move2));
         inc(zuege);
         p2l5.caption:=inttostr(zuege);
         l9.caption:=floattostrf(100.0*zuege/gesamtzuege,ffgeneral,4,2);
         gefunden:=false;
...
begin
    ...
    begin
      ...
      roundrect(b div 3,h-unten,pb1.width-(b div 3),h-unten+20,20,20);
      brush.color:=$00c0c080;
      for i:=1 to 3 do rectangle(i*b-4,h-unten-scheibenzahl*shoehe-10,
                                  i*b+5,h-unten+1);
      brush.style:=bsclear;
...
begin
    ...
    inc(stab[tobar,0]);
      ...
      if ani then
      begin
        for w:=a1*shoehe to scheibenzahl*shoehe+www do
          anibit(tobar,a2+1,frombar*b,w);
        if frombar<tobar then
          for w:=frombar*b to tobar*b do
            anibit(tobar,a2+1,w,scheibenzahl*shoehe+www)
        else
          for w:=frombar*b downto tobar*b do
            anibit(tobar,a2+1,w,scheibenzahl*shoehe+www);
        for w:=scheibenzahl*shoehe+www downto (a2+1)*shoehe do
          anibit(tobar,a2+1,tobar*b,w);
        p2l5.caption:=inttostr(zuege);
        zeit2:=gettickcount;
...
begin
   ...
   begin
     ...
     begin
       ...
       if bele and (not sabbruch) then
        lb2.items.add(inttostr(zuege+1)+#9+chr(64+frombar)+' nach '+chr(64+tobar));
       pai;
...
procedure TForm1.d2click4(Sender: TObject);
var
  b,h,nr,scheiben,verz:integer;
  zeit,zeit2:integer;
  ani,bele:boolean;
...
begin
    ...
    begin
      ...
      roundrect(b div 4,h-unten,pb1.width-(b div 4),h-unten+20,20,20);
      brush.color:=$00c0c080;
      for i:=1 to 4 do rectangle(i*b-4,h-unten-scheibenzahl*shoehe-10,
                                  i*b+5,h-unten+1);
      brush.style:=bsclear;
...
begin
   ...
   begin
     ...
     begin
       MoveTower(FromBar, TempBar, ToBar, nDisc-1);
       if bele and (sabbruch=false) then
        lb2.items.add(inttostr(zuege+1)+#9+chr(64+frombar)+' to '+chr(64+tobar));
       pai;
       MoveTower(TempBar, ToBar, FromBar, nDisc-1);
     end
     else
...
procedure TForm1.lb2Click(Sender: TObject);
begin
