...
uses
  ...
  Buttons, ComCtrls, ExtCtrls, StdCtrls, Menus, RXSpin, RxGIF, Mask;
...
var
  li,ob,xx,yy,i,j,stack_:integer;
...
begin
    ...
    begin
      ...
      begin
        showmessage('You lost to the computer! Restart using the switch.');
      end;
    end;

    if sieger>0 then
    begin
      showmessage('Player '+inttostr(sieger)+' Wins! Restart via switch');
    end
    else
    begin
      if zuege>feld2 then
...
var
  g,i,j,w,maxx,xx,yy,anz:integer;
...
begin
    li:=(PaintBox1.width-xb*feld) div 2;
    ob:=(PaintBox1.height-xb*feld) div 2;
    xx:=(x-li+xb) div xb;
    yy:=(y-ob+xb) div xb;
...
procedure tForm1.zeichnen(sender: tobject);
var
  Bitmap: TBitmap;
  myrect,birect:trect;
  b,h:integer;
    ...
    for i:=0 to feld-1 do
      ...
      begin
        ...
        case (belegung[i+1,j+1] mod 10) of
          1 : begin
                bitmap.canvas.ellipse(li+i*xb+xc+2,
                                      ob+j*xb+xc+2,
                                      li+i*xb+xb-xc-1,
                                      ob+j*xb+xb-xc-1);
                inc(gesamt);
          ...
          2 : begin
                inc(gesamt,2);
                bitmap.canvas.ellipse(li+i*xb+4,
                                      ob+j*xb+4,
                                      li+i*xb+xb-15,
                                      ob+j*xb+xb-15);
                bitmap.canvas.ellipse(li+i*xb+xb-24,
                                      ob+j*xb+xb-24,
                                      li+i*xb+xb-3,
                                      ob+j*xb+xb-3);
              end;
          3..6 : begin
                inc(gesamt,3);
                bitmap.canvas.ellipse(li+i*xb+10,
                                      ob+j*xb+3,
                                      li+i*xb+xb-9,
                                      ob+j*xb+xb-16);
                bitmap.canvas.ellipse(li+i*xb+3,
                                      ob+j*xb+17,
                                      li+i*xb+22,
                                      ob+j*xb+xb-2);
                bitmap.canvas.ellipse(li+i*xb+17,
                                      ob+j*xb+17,
                                      li+i*xb+xb-2,
                                      ob+j*xb+xb-2);
              end;
      ...
      bitmap.canvas.TextOut(42,8,'next player');
