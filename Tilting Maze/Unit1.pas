...
uses
  ...
  ExtCtrls, Menus, StdCtrls, ComCtrls, Buttons, XPMan;
...
begin
    ...
    Label2.caption:=inttostr(sel)+' Games';
    if nr>sel then nr:=1;
    k:='['+inttostr(nr);
...
var
  fb,i,j,b,h,xoffset,yoffset:integer;
  bild:tbitmap;
begin
    ...
    if doppelspiel then begin
      if ((sx=zx) and (sy=zy) and (sx2=zx2) and (sy2=zy2)) or
        ...
        showmessage('Congratulations! You have solved the task.');
        inc(nr);
        edit1.text:=inttostr(nr);
      end;
    end
    else begin // no double play
