...
uses
  ...
  Buttons, StdCtrls, ExtCtrls, XPMan;
...
procedure TForm1.I1D(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  i,j:integer;
begin
...
procedure TForm1.I1U(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  qx,qy,nrx,nry,i,j,f,z,z2:integer;
  geloest:boolean;
begin
  nrx:=0;
  nry:=0;
  ...
  begin
    qx:=ix+x;
    qy:=iy+y;
    if (qx>=p1.left) and (qx<=p1.left+380) and (qy>=80) and (qy<=480) then
    begin
      qx:=ix+x;
    ...
    if (qy>=80) and (qy<=480) and (qx>=p1.left) and (qx<=p1.left+380) then
    begin
      qy:=iy+y;
      nry:=1+(qy-88) div 95;
    ...
    if (nrx in [1..4]) and (nry in [1..4]) then
    begin
      if pp[nrx,nry]=0 then
      begin
        pp[nrx,nry]:=ii;
      end else begin
        qx:=ix+x-px;
...
//test
    ...
    l1.caption:=inttostr(f)+' Error';
    geloest:=f=0;
    for i:=1 to 4 do
      for j:=1 to 4 do
        if pp[i,j]=0 then geloest:=false;
    if geloest then showmessage('Congratulations! The puzzle has been solved.!');
  end;
end;

procedure TForm1.I1M(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if ii<>0 then
  begin
    ...
    if im[ii].left<=0 then
      im[ii].left:=0;
    if im[ii].top<=0 then
      im[ii].top:=0;
    if im[ii].left>=p8.width-96 then
      im[ii].left:=p8.width-96;
    if im[ii].top>=p8.height-96-p7.height then
      im[ii].top:=p8.height-96-p7.height;
    ix:=im[ii].left;
    iy:=im[ii].top;
  end;
end;
