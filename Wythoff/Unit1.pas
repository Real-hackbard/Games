...
uses
  ...
  ExtCtrls, StdCtrls, Menus, ComCtrls, Buttons, Spin, Math, XPman;
...
begin
  ...
  begin
    ...
    if moves[aktuell].row<squares then
    begin
    ...
    if (moves[aktuell].row<squares) or (moves[aktuell].col>0) then
    begin
      i:=moves[aktuell].col-1;
...
procedure TForm1.PaintBox1MouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
var
  i,m,n,b,h,xoffset,yoffset:integer;
  korrekt:boolean;
begin
  b:=PaintBox1.width;
  h:=PaintBox1.height;
...
procedure TForm1.ausgabe(s:boolean; sender:tobject);
var
  z:integer; k1,k2:string[5];
begin
  ...
  if s then
    ListBox1.items.add('Player'#9+'('+k1+'|'+k2+')')
  else
    ListBox1.items.add('Computer'#9+'('+k1+'|'+k2+')');
end;
...
begin
  ...
  begin
    if (moves[aktuell].col-round((m-x)/w)>=0)
       and (moves[aktuell].row-round((n-y)/w)<=squares) then
       begin
      aktuell:=aktuell+1;
      ...
      ausgabe(true,sender);
      PaintBox1paint(sender);
      if (moves[aktuell].col=0) and (moves[aktuell].row=squares) then
      begin
        timer1.enabled:=false; showmessage('Congratulations! Youve won!!');
      end
      else computer:=true;
    end;
  end;
...
begin
  test:=false;
  if (zug[z].x=a) and (b<zug[z].y) then
  begin
    test:=true;
    exit
  end;

  if (zug[z].y=b) and (a>zug[z].x) then
  begin
    test:=true;
    exit
  end;
