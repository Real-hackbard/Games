...
begin
    ...
    bitmap.height:= PaintBox1.height+40;
    li:=(PaintBox1.width-704) div 2;
    ...
    bitmap.canvas.textout(70,580,'Diamonds '+inttostr(punktmax));
    bitmap.canvas.textout(480,580,'collected diamonds '+inttostr(punkte));

    bitmap.canvas.textout(70,50,'Game level '+inttostr(stufe));
    if lebt then begin
       datetimetostring(k,'n:ss',zeit0-time);
       bitmap.canvas.textout(340,50,'Time '+k);
    end;
    bitmap.canvas.textout(600,50,'Points '+inttostr(gesamtpunkte));

    PaintBox1.canvas.draw(0,-30,bitmap);
    bitmap.free;
...
procedure TForm1.stufeein(Sender: TObject);
type
...
begin
    ...
    if endgeschafft then Zurcksetzen1.caption:='Reset'
                    else dec(leben);
...
procedure TForm1.test(a,b:integer;Sender: TObject);
var
  alt:integer;
begin
    if lebt then
    begin
      ...
      begin
        ...
        begin
          showmessage('Congratulations! You have successfully completed all levels!');
          stufe:=1;
        end
        else
        begin
          if leben=0 then leben:=1;
          showmessage('Congratulations! You have reached level '+inttostr(stufe-1)+
...
procedure TForm1.schluss(Sender: TObject);
begin
    showmessage('Game over. They reached '+inttostr(gesamtpunkte)+' Points !');
    Zurcksetzen1.caption:='Restart';
    gesamtpunkte:=0;
end;
